...
function startGame(level) {
    ...
    document.getElementById("task").textContent = `Напишите число ${currentNumber} на немецком языке:`;
    document.getElementById("answer").value = '';
    document.getElementById("feedback").textContent = '';
    document.getElementById("check-button").textContent = 'Проверить';
...
function checkAnswer(event) {
    ...
    if (answer === "") {
        document.getElementById("feedback").textContent = "Введите ответ.";
        document.getElementById("feedback").className = 'error';
        document.getElementById("answer").focus();
        return;
    }

    const correctAnswer = numberToGerman(currentNumber).toLowerCase();
    const feedbackElement = document.getElementById("feedback");
    if (answer === correctAnswer) {
        feedbackElement.textContent = `Правильный ответ!`;
        feedbackElement.className = 'correct';
        document.getElementById("check-button").textContent = 'Продолжить';
        document.getElementById("check-button").onclick = nextNumber;
        incorrectAttempts = 0;
    } else {
        incorrectAttempts++;
        if (incorrectAttempts >= 3) {
            feedbackElement.textContent = `Неправильный ответ! Правильный ответ: ${correctAnswer}`;
            feedbackElement.className = 'error';
        } else {
            switch (incorrectAttempts) {
                case 1:
                    feedbackElement.textContent = `Неправильный ответ! Попробуйте снова.`;
                    break;
                case 2:
                    feedbackElement.textContent = `Все еще неправильный ответ!`;
                    break;
            }
            feedbackElement.className = 'error';
        }
...
function handleKeyDown(event) {
    if (event.key === "Enter" || event.key === "Return") {
        ...
        if (document.getElementById("check-button").textContent === "Продолжить") {
            nextNumber();
        } else {
            checkAnswer();
        }
