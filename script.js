...
function startGame(level) {
    ...
    document.getElementById("task").textContent = `Schreiben Sie die Zahl ${currentNumber} auf Deutsch:`;
...
function checkAnswer(event) {
    ...
    if (answer === correctAnswer) {
        feedbackElement.textContent = `Richtige Antwort!!`;
...
function handleKeyDown(event) {
    if (event.key === "Enter" || event.key === "Return") {
        ...
        if (document.getElementById("check-button").textContent === "Weiter") {
            nextNumber();
        } else {
            checkAnswer();
        }
